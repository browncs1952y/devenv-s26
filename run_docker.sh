...
CONTAINER="cs1952y-container"
IMAGE="cs1952y:latest"

cd "$(dirname "$0")" || exit

if [ ! -d ../Ripes ]; then
    echo "Directory ../Ripes ($(realpath ../Ripes)) does not exist. Have you run setup_docker.sh?"
    exit
fi
if [ ! -d ../gem5 ]; then
...
if $clean; then
    ...
    existing_container=$(docker ps -f status=running -f ancestor="$IMAGE" -f volume="/home/cs1952y-user/gem5" -f volume="/home/cs1952y-user/Ripes" --no-trunc --format "{{.CreatedAt}},{{.ID}}" | sort -r | head -n 1)
